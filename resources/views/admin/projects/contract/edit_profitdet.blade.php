@foreach ($contract_units_details as $unitkey => $unitDetail)
    @php
        $unit_rent = (float) (toNumeric($unitDetail->unit_rent_per_annum) ?? 0);
        $unitComm = (float) ($unit_rent * ($unitDetail->unit_commission ?? 0)) / 100;
        $unitDepo = (float) ($unit_rent * ($unitDetail->unit_deposit ?? 0)) / 100;

        $unit_payable = (float) ($unit_rent + $unitComm + $unitDepo);
    @endphp
